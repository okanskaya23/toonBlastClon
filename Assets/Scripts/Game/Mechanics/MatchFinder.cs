...
namespace Game.Mechanics
{
    public class MatchFinder
    {
        ...
        public void FindAndAddAdjacentBreakables(List<Cell> cells)
        {
            List<Cell> newBreakables = new List<Cell>();

            for (var i = 0; i < cells.Count; i++)
            {
                var neighbours = cells[i].Neighbours;

                foreach (Cell neighbour in neighbours)
                {
                    if (neighbour.Item == null) continue;

                    if (neighbour.Item.IsAdjacentBreakable() && !newBreakables.Contains(neighbour)
                        && (neighbour.Item.GetMatchType() == MatchType.None || neighbour.Item.GetMatchType() == cells[i].Item.GetMatchType())) // && (neighbour.Item.GetMatchType() == MatchType.None || neighbour.Item.GetMatchType() == cells[i].Item.GetMatchType())
                    {
                        newBreakables.Add(neighbour);
                    }
                }
            }
            cells.AddRange(newBreakables);
        }
